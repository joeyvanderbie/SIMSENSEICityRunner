...
import org.hva.cityrunner.data.AmenityType;
import org.hva.cityrunner.data.LatLon;
import org.hva.cityrunner.osm.MapRenderingTypes;
import org.hva.cityrunner.util.Algorithms;
import org.hva.cityrunner.util.OpeningHoursParser;
import org.hva.cityrunner.util.OpeningHoursParser.OpeningHours;

import org.hva.cityrunner.access.AccessibleToast;
import org.hva.cityrunner.access.NavigationInfo;
import org.hva.cityrunner.plus.NameFinderPoiFilter;
import org.hva.cityrunner.plus.OsmAndFormatter;
...
import org.hva.cityrunner.plus.SearchByNameFilter;
import org.hva.cityrunner.plus.OsmAndLocationProvider.OsmAndCompassListener;
import org.hva.cityrunner.plus.OsmAndLocationProvider.OsmAndLocationListener;
import org.hva.cityrunner.plus.activities.EditPOIFilterActivity;
import org.hva.cityrunner.plus.activities.MapActivity;
import org.hva.cityrunner.plus.activities.MapActivityActions;
import org.hva.cityrunner.plus.activities.OsmandListActivity;
import org.hva.cityrunner.plus.render.RenderingIcons;
...
public class SearchPOIActivity extends OsmandListActivity implements OsmAndCompassListener, OsmAndLocationListener {
	...
	public static final String AMENITY_FILTER = "org.hva.cityrunner.amenity_filter"; //$NON-NLS-1$
	...
	protected void onResume() {
		...
		if(bundle.containsKey(SEARCH_LAT) && bundle.containsKey(SEARCH_LON)){
			location = new org.hva.cityrunner.Location("internal"); //$NON-NLS-1$
	...
	private void showPoiCategoriesByNameFilter(String query, org.hva.cityrunner.Location loc){
	...
	private org.hva.cityrunner.Location getSearchedLocation(){
	...
	@Override
	public void updateLocation(org.hva.cityrunner.Location location) {
		boolean handled = false;
		if (location != null && filter != null) {
			org.hva.cityrunner.Location searchedLocation = getSearchedLocation();
	...
	static class SearchAmenityRequest {
		...
		private int type;
		private org.hva.cityrunner.Location location;
		
		public static SearchAmenityRequest buildRequest(org.hva.cityrunner.Location l, int type){
	...
	class SearchAmenityTask extends AsyncTask<Void, Amenity, List<Amenity>> implements ResultMatcher<Amenity> {
		...
		org.hva.cityrunner.Location getSearchedLocation(){
	...
	class AmenityAdapter extends ArrayAdapter<Amenity> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			org.hva.cityrunner.Location loc = location;
			if(loc != null){
				mes = new float[2];
				LatLon l = amenity.getLocation();
				org.hva.cityrunner.Location.distanceBetween(l.getLatitude(), l.getLongitude(), loc.getLatitude(), loc.getLongitude(), mes);
			}
			int opened = -1;
			if (amenity.getOpeningHours() != null) {
