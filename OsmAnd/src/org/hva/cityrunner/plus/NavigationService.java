...
public class NavigationService extends Service implements LocationListener {
	...
	private void checkForegroundAPI() {
		...
		try {
			...
			Log.d(PlatformUtilCityRunner.TAG, "startForeground and stopForeground available");
		} catch (NoSuchMethodException e) {
			mStartForeground = null;
			mStopForeground = null;
			Log.d(PlatformUtilCityRunner.TAG, "startForeground and stopForeground not available");
		}

		// check old API
		try {
			mSetForeground = getClass().getMethod("setForeground", new Class[] {boolean.class});
			Log.d(PlatformUtilCityRunner.TAG, "setForeground available");
		} catch (NoSuchMethodException e) {
			mSetForeground = null;
			Log.d(PlatformUtilCityRunner.TAG, "setForeground not available");
		}
	}
...
//		}
	...
	private void showNotificationInStatusBar(ClientContext cl) {
		...
		if (mStartForeground != null) {
			Log.d(PlatformUtilCityRunner.TAG, "invoke startForeground");
			try {
				mStartForeground.invoke(this, NOTIFICATION_SERVICE_ID, notification);
			} catch (InvocationTargetException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke startForeground failed");
			} catch (IllegalAccessException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke startForeground failed");
			}
		} else {
			Log.d(PlatformUtilCityRunner.TAG, "invoke setForeground");
			mNotificationManager.notify(NOTIFICATION_SERVICE_ID, notification);
			try {
				mSetForeground.invoke(this, Boolean.TRUE);
			} catch (InvocationTargetException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke setForeground failed");
			} catch (IllegalAccessException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke setForeground failed");
			}
		}
	...
	private void removeNotification() {
		...
		if (mStopForeground != null) {
			Log.d(PlatformUtilCityRunner.TAG, "invoke stopForeground");
			try {
				mStopForeground.invoke(this, Boolean.TRUE);
			} catch (InvocationTargetException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke stopForeground failed");
			} catch (IllegalAccessException e) {
				Log.d(PlatformUtilCityRunner.TAG, "invoke stopForeground failed");
			}
		}
	...
	public void onLocationChanged(Location l) {
		if(l != null && !settings.MAP_ACTIVITY_ENABLED.get()){
			org.hva.cityrunner.Location location = OsmAndLocationProvider.convertLocation(l,(OsmandApplication) getApplication());
			if(!isContinuous()){
