...
import org.hva.cityrunner.GeoidAltitudeCorrection;
import org.hva.cityrunner.binary.RouteDataObject;
...
import org.hva.cityrunner.access.NavigationInfo;
import org.hva.cityrunner.plus.OsmandSettings.OsmandPreference;
...
public class OsmAndLocationProvider implements SensorEventListener {
	...
	public interface OsmAndLocationListener {
		void updateLocation(org.hva.cityrunner.Location location);
	}
	...
	private org.hva.cityrunner.Location location = null;
	...
	public void resumeAllUpdates() {
		...
		try {
			...
			Log.d(PlatformUtilCityRunner.TAG, "GPS location provider not available"); //$NON-NLS-1$
		}
		...
		try {
			...
			Log.d(PlatformUtilCityRunner.TAG, "Network location provider not available"); //$NON-NLS-1$
		}
	}
	...
	public org.hva.cityrunner.Location getFirstTimeRunDefaultLocation() {
		...
		for (String provider : providers) {
			org.hva.cityrunner.Location location = convertLocation(service.getLastKnownLocation(provider), app);
			if (location != null) {
				return location;
			}
		}
		...
		if (sensorRegistered && !register) {
			...
			if (USE_MAGNETIC_FIELD_SENSOR_COMPASS.get()) {
				...
				if (s == null || !sensorMgr.registerListener(this, s, SensorManager.SENSOR_DELAY_UI)) {
					Log.e(PlatformUtilCityRunner.TAG, "Sensor accelerometer could not be enabled");
				}
				s = sensorMgr.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);
				if (s == null || !sensorMgr.registerListener(this, s, SensorManager.SENSOR_DELAY_UI)) {
					Log.e(PlatformUtilCityRunner.TAG, "Sensor magnetic field could not be enabled");
				}
			} else {
				Sensor s = sensorMgr.getDefaultSensor(Sensor.TYPE_ORIENTATION);
				if (s == null || !sensorMgr.registerListener(this, s, SensorManager.SENSOR_DELAY_UI)) {
					Log.e(PlatformUtilCityRunner.TAG, "Sensor orientation could not be enabled");
				}
			}
	...
	// location not null!
	private void updateSpeedEmulator(org.hva.cityrunner.Location location) {
	...
	public static boolean isPointAccurateForRouting(org.hva.cityrunner.Location loc) {
	...
	private float calcGeoMagneticCorrection(float val) {
		if (previousCorrectionValue == 360 && getLastKnownLocation() != null) {
			org.hva.cityrunner.Location l = getLastKnownLocation();
...
//		}
	...
	private void updateLocation(org.hva.cityrunner.Location loc ) {
	...
	public static org.hva.cityrunner.Location convertLocation(Location l, OsmandApplication app) {
		if (l == null) {
			return null;
		}
		org.hva.cityrunner.Location r = new org.hva.cityrunner.Location(l.getProvider());
	...
	private void scheduleCheckIfGpsLost(org.hva.cityrunner.Location location) {
		...
		if (location != null) {
			...
			app.runMessageInUIThreadAndCancelPrevious(LOST_LOCATION_MSG_ID, new Runnable() {
				...
				@Override
				public void run() {
					org.hva.cityrunner.Location lastKnown = getLastKnownLocation();
	...
	public void setLocationFromService(org.hva.cityrunner.Location location, boolean continuous) {
	...
	public void setLocationFromSimulation(org.hva.cityrunner.Location location) {
		setLocation(location);
	}

	private void setLocation(org.hva.cityrunner.Location location) {
		if(location == null){
			updateGPSInfo(null);
		}
		...
		// 3. routing
		org.hva.cityrunner.Location updatedLocation = location;
	...
	private void enhanceLocation(org.hva.cityrunner.Location location) {
	...
	public void checkIfLastKnownLocationIsValid() {
		org.hva.cityrunner.Location loc = getLastKnownLocation();
	...
	public org.hva.cityrunner.Location getLastKnownLocation() {
		return location;
	}
