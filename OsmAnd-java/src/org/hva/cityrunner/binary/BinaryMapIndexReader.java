package org.hva.cityrunner.binary;
...
import org.hva.cityrunner.Collator;
import org.hva.cityrunner.CollatorStringMatcher;
import org.hva.cityrunner.CollatorStringMatcher.StringMatcherMode;
import org.hva.cityrunner.PlatformUtilCityRunner;
import org.hva.cityrunner.ResultMatcher;
import org.hva.cityrunner.StringMatcher;
import org.hva.cityrunner.binary.BinaryMapAddressReaderAdapter.AddressRegion;
import org.hva.cityrunner.binary.BinaryMapAddressReaderAdapter.CitiesBlock;
import org.hva.cityrunner.binary.BinaryMapPoiReaderAdapter.PoiRegion;
import org.hva.cityrunner.binary.BinaryMapRouteReaderAdapter.RouteRegion;
import org.hva.cityrunner.binary.BinaryMapRouteReaderAdapter.RouteSubregion;
import org.hva.cityrunner.binary.BinaryMapTransportReaderAdapter.TransportIndex;
import org.hva.cityrunner.binary.OsmandOdb.MapDataBlock;
import org.hva.cityrunner.binary.OsmandOdb.OsmAndMapIndex.MapDataBox;
import org.hva.cityrunner.binary.OsmandOdb.OsmAndMapIndex.MapEncodingRule;
import org.hva.cityrunner.binary.OsmandOdb.OsmAndMapIndex.MapRootLevel;
...
import org.hva.cityrunner.data.TransportRoute;
import org.hva.cityrunner.data.TransportStop;
...
public class BinaryMapIndexReader {
	...
	private final static Log log = PlatformUtilCityRunner.getLog(BinaryMapIndexReader.class);
	 ...
	 * Transport public methods
	 */
	public List<org.hva.cityrunner.data.TransportRoute> getTransportRouteDescriptions(TransportStop stop) throws IOException {
		TransportIndex ind = getTransportIndex(stop.getFileOffset());
		if(ind == null){
			return null;
		}
		List<org.hva.cityrunner.data.TransportRoute> list = new ArrayList<TransportRoute>();
		TIntObjectHashMap<String> stringTable = new TIntObjectHashMap<String>();
...
//		}
	...
	public Map<AmenityType, List<String> > searchPoiCategoriesByName(String query, Map<AmenityType, List<String> > map) throws IOException {
		...
		Collator collator = PlatformUtilCityRunner.primaryCollator();
		for (PoiRegion poiIndex : poiIndexes) {
			poiAdapter.initCategories(poiIndex);
	...
	private static void testTransportSearch(BinaryMapIndexReader reader) throws IOException {
		...
		}
		{
			for (TransportStop s : reader.searchTransportIndex(buildSearchTransportRequest(sleft, sright, stop, sbottom, 15, null))) {
				...
				for (org.hva.cityrunner.data.TransportRoute route : routes.valueCollection()) {
